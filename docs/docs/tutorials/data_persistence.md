...
このチュートリアルでは、DBのデータに対する永続化の方法について紹介します。<br>
...
`sdts_open_db` APIにて、オプションのDB_PATHを指定することで、永続化DBを作成します。<br>
DB_PATHを指定しない場合は、メモリDBのみになります。<br>
DB_PATHに指定した場所にDBファイルがない場合は、新規にDBのファイル構造を作成します。<br>
既にあるDBファイルを指定した場合は、永続化されたDBを読み込みます。<br>
...
メモリ上にあるデータを永続化するために同期処理を行います。<br>
