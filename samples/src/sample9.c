...
static int cursor_aggr(sdtsdb_t db, sdtscid_t *cids,
	int ccnt, sdntime_t st, sdntime_t et, sdntime_t iv, ui32_t opt) {
	sdtscur_t cur;
	sdtscurval_t val[CCNT];
	int i, j, ret;
	sdntime_t ts;

	if ((cur = sdts_open_cur(db, cids, ccnt, st, et, iv, opt)) < 0) {
...
int main(int ac, char *av[]) {
	...
	opt = 0;		// default
	if (cursor_aggr(db, cids, CCNT, st, et, iv, opt) < 0) {
		(void)sdts_close_db(db);
		sd_end();
		return 1;
	}
	...
	//  区間内に値が存在しない場合、以前の値を引継ぐ
	st = 1001100000;
	et = 1004100000;
	iv = 500000;	// aggr interval 500us
	opt = SDTS_CUR_OPT_AGGR_VAL_CONT;
	if (cursor_aggr(db, cids, CCNT, st, et, iv, opt) < 0) {
		(void)sdts_close_db(db);
		sd_end();
		return 1;
	...
	st = 1001101000;
	et = 1004101000;
	iv = 200000;	// aggr interval 500us
	opt = SDTS_CUR_OPT_AGGR_TS_PREV | SDTS_CUR_OPT_AGGR_VAL_TOP;
	if (cursor_aggr(db, cids, CCNT, st, et, iv, opt) < 0) {
		(void)sdts_close_db(db);
		sd_end();
		return 1;
