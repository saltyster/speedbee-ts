...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		int ret = spcf_get_col_dat(db, col_params[2].cid, SDTS_STO_OPT_MEM|SDTS_STO_OPT_FILE, ts, &nxt, &hdr, (uint8_t*)buff, BSZ, &outc);
		if (ret == SPCF_ERROR) {
			printf("SPCF_ERROR [%d]\n" , sd_get_err()); break;
	...
	printf("-- success spcf_get_col_dat --\n");
